...
RSpec.describe EditionLinkExtractor do
  context ".call" do
    let(:edition_with_no_links) { FactoryGirl.create(:travel_advice_edition) }
    let(:edition_with_links_in_govspeak_fields) { FactoryGirl.create(:travel_advice_edition, summary: "This is a [link](https://www.example.com)") }
    let(:edition_with_links_in_parts) { FactoryGirl.create(:travel_advice_edition_with_parts) }
    let(:edition_with_absolute_paths_in_govspeak_fields) { FactoryGirl.create(:travel_advice_edition, summary: "This is a [link](https://www.example.com). This is an absolute [path](/id-for-driving-licence)") }
